...
class match_info():
    ...
    def calc_age(player_id, period_id, db):
        ...
        birthday = pd.to_datetime(
            pd.read_sql_query(birthday_query, db).iloc[0]['birthday']
        )

        period_start = pd.to_datetime(
            pd.read_sql_query(period_query, db).iloc[0]['start']
        )

        if birthday == None:
            birthday = replacement_birthdays[player_id]
        else:
            assert birthday != None
    ...
    def clean_df(df):
        ...
        Returns:
            cleaned DataFrame
        """
        # Create effective ratings
        match_info.create_eff_ratings('a', df)
        match_info.create_eff_ratings('b', df)

        # calculate ratings differences and add a higher ranked column
        df = match_info.process_calcs(df)
    ...
    def create_input(player_a, player_b, start_date, db):
        """
        Takes two player tags and outputs the cleaned dataframe for model input.
        """

        # grab period first
        start, end = match_info.create_date_str(start_date)
        period_id = match_info.query_period(start, end, db)

        if period_id == None:
            return('use an earlier date')

        # get each player's info
        player_a_df = match_info.gather_player_df(player_a, period_id, db)
        player_b_df = match_info.gather_player_df(player_b, period_id, db)

        # combine dfs
        match_df = match_info.combine_player_df(player_a_df, player_b_df)

        # call cleaning function
        match_df = match_info.clean_df(match_df)
